...
/datum/disease/proc/getImmunityLossMultiplier(var/division_adjustment)
	if(istype(affected_mob, /mob/living/carbon))
		var/blood_loss = 100 - round((affected_mob.blood_volume / BLOOD_VOLUME_NORMAL)*100)
		var/immunityLoss = ((affected_mob.getToxLoss() * 1.75) + (blood_loss * 1.5) + (affected_mob.getBruteLoss() * 1.25) + affected_mob.getFireLoss()) / 120
